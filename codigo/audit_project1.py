...
required = [
    "entregables/cuaderno/proyecto1_calderon_barillas.ipynb",
    "entregables/informe/informe.tex", "entregables/informe/informe.pdf",
...
nb = nbformat.read(root / "entregables/cuaderno/proyecto1_calderon_barillas.ipynb", as_version=4)
...
report_pages = len(pymupdf.open(root / "entregables/informe/informe.pdf"))
presentation_pages = len(pymupdf.open(root / "entregables/presentacion/presentacion.pdf"))
assert report_pages <= 7, report_pages
assert presentation_pages == 8, presentation_pages

html = (root / "entregables/presentacion/presentacion.html").read_text(encoding="utf-8")
...
loose_root_files = [p.name for p in root.iterdir() if p.is_file()]
assert not loose_root_files, f"Archivos sueltos en raíz: {loose_root_files}"

docx_path = root / "entregables/ficha/Ficha_Repositorio_Proyecto1.docx"
