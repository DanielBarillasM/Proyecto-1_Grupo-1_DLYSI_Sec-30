...
def main() -> int:
    ...
    expected = [
        ".github/README.md",
        "codigo/dataset_v3_support.py", "codigo/proyecto1_v3_pipeline.py",
        "codigo/postprocess_v3.py", "codigo/finalize_v3.py",
        "codigo/build_v3_deliverables.py", "codigo/audit_project1_v3.py",
        ...
        "entregables/cuaderno/Proyecto_1_Monitoreo_Transaccional_V3.ipynb",
        "entregables/informe/informe_proyecto1_v3.tex", "entregables/informe/informe_proyecto1_v3.pdf",
        "entregables/presentacion/presentacion_proyecto1_v3.html", "entregables/presentacion/presentacion_proyecto1_v3.pdf",
        "entregables/ficha/Ficha_Repositorio_Proyecto_1_V3.docx", "entregables/ficha/Ficha_Repositorio_Proyecto_1_V3.pdf",
    ]
    for relative in expected:
        check((ROOT / relative).exists(), f"Falta {relative}")
    check(not [p.name for p in ROOT.iterdir() if p.is_file()], "Hay archivos sueltos en la raíz")
    ...
    notebook_path = ROOT / "entregables/cuaderno/Proyecto_1_Monitoreo_Transaccional_V3.ipynb"
    ...
    report_pdf = ROOT / "entregables/informe/informe_proyecto1_v3.pdf"
    if report_pdf.exists():
        pages = len(fitz.open(report_pdf))
        check(5 <= pages <= 7, f"Informe V3 tiene {pages} páginas; se esperan 5–7")
    slides_html = ROOT / "entregables/presentacion/presentacion_proyecto1_v3.html"
    ...
    slides_pdf = ROOT / "entregables/presentacion/presentacion_proyecto1_v3.pdf"
    if slides_pdf.exists():
        check(len(fitz.open(slides_pdf)) == 8, "La presentación PDF no tiene ocho páginas")
    ficha_pdf = ROOT / "entregables/ficha/Ficha_Repositorio_Proyecto_1_V3.pdf"
    if ficha_pdf.exists():
