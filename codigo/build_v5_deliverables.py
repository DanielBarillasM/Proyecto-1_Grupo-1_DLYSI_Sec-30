...
def build_notebook() -> Path:
    out_dir = ROOT / "entregables" / "cuaderno"
...
\vfill\textbf{{Declaración de IA.}} Se usó asistencia para código, redacción, visualización y auditoría. Los autores ejecutaron y verificaron datos, métricas, falsificaciones y conclusiones.\end{{document}}"""
    out_dir = ROOT / "entregables" / "informe"
    out_dir.mkdir(parents=True, exist_ok=True)
...
def build_slides() -> Path:
    slides = [
        ("DECISIÓN", "Conservar A; el orden no demuestra valor", f'<div class="big">AP {f(INTERNAL["A_V4"]["auc_pr"])}</div><div class="call">A minimiza costo interno · revisión humana</div>', "Respuesta ejecutiva: no migrar a secuencias con la evidencia actual."),
        ("DATOS", "590,540 eventos; futuro fuera", '<div class="cols"><div class="box"><b>70·15·15</b><p>train · validación · benchmark</p></div><div class="box"><b>16</b><p>eventos máximos por secuencia</p></div></div><div class="call">Identidad proxy: tarjeta + dirección. Preprocesamiento train-only.</div>', "El benchmark ya fue observado y se declara histórico reutilizado."),
        ("DISEÑO", "Una comparación común A/B/C", '<div class="road"><span>A · LightGBM expertos</span><span>B · GRU(64)</span><span>C · stacking A+B</span></div><p class="formula">sᵢ=P(yᵢ=1 | x₁,…,xᵢ)</p><div class="call">Mismas filas, partición, horizonte y costo.</div>', "C tiene hipótesis y criterio fijados antes de su evaluación interna."),
        ("RESULTADOS", "A gana; C no cumple su apuesta", f'<div class="cols"><div class="box accent"><b>A · {f(INTERNAL["A_V4"]["auc_pr"])}</b><p>{money(INTERNAL["A_V4"]["cost_q"])}</p></div><div class="box"><b>B · {f(INTERNAL["B_GRU"]["auc_pr"])}</b><p>C · {f(INTERNAL["C_fusion"]["auc_pr"])}</p></div></div><img src="../../evidencia/figuras/v5/01_comparacion_abc_validacion.png">', "C pierde AP y aumenta costo frente a A; veredicto no útil."),
        ("FALSIFICACIÓN", "Permutar mejora, no destruye, el ranking", f'<img src="../../evidencia/figuras/v5/03_falsificaciones_orden_v5.png"><div class="call">ΔAP original−permutada = {f(FALS["order_auc_pr_drop"],4)}</div>', "No afirmar que el orden aporta. La historia recortada tampoco cae materialmente."),
        ("ECONOMÍA", "Q4,200 por FN cambia la decisión", f'<div class="cols"><div class="box accent"><b>{R["umbrales"]["A_V4"]:.4f}</b><p>umbral A</p></div><div class="box"><b>{money(BENCH["A_V4"]["cost_q"])}</b><p>benchmark histórico</p></div></div><img src="../../evidencia/figuras/v5/04_costos_abc_v5.png">', "Umbral elegido antes de evaluación interna; escenario mensual no es cifra contable."),
        ("ERRORES", "Priorizar FN de alto monto", '<div class="road"><span>monto</span><span>producto</span><span>historial corto</span><span>deriva</span></div><div class="call">El puntaje ordena revisión; no prueba fraude ni autoriza bloqueo.</div>', "Discutir identidad proxy, deriva y capacidad de alertas."),
        ("SIGUIENTE PASO", "Congelar A y obtener cohorte nueva", '<div class="road"><span>identidad fiable</span><span>costos reales</span><span>privacidad</span><span>latencia</span><span>apelación</span></div><div class="call">Candidato al Proyecto Final: A con revisión humana.</div>', "No API, MLflow ni dashboard en Proyecto 1; sí contrato y artefactos reutilizables."),
    ]
    css = """*{box-sizing:border-box}body{margin:0;background:#081a2a;color:#eef7ff;font-family:Inter,'Segoe UI',sans-serif;overflow:hidden}.slide{display:none;width:100vw;height:100vh;padding:5.5vh 7vw;background:radial-gradient(circle at 90% 10%,#2a9d8f55,transparent 30%),linear-gradient(135deg,#102a43,#091b2b);position:relative}.slide.active{display:block}.eye{color:#65d3c3;font-weight:900;letter-spacing:.16em;font-size:1.2vw}h1{font-size:3.8vw;line-height:1.06;margin:2vh 0 3vh}p,li{font-size:1.45vw;line-height:1.4}.content{height:69vh}.big{font-size:7vw;font-weight:900;color:#65d3c3;line-height:1}.cols{display:grid;grid-template-columns:1fr 1fr;gap:2.2vw}.box{padding:1.5vw;border:1px solid #ffffff33;border-radius:1.2vw;background:#ffffff0d}.box b{font-size:2.5vw;color:#65d3c3}.accent{border-color:#65d3c3}.call{padding:1vw;margin-top:1.5vh;border-left:.5vw solid #2a9d8f;background:#ffffff12;border-radius:.8vw;font-size:1.3vw}img{display:block;max-width:75%;max-height:42vh;margin:1.2vh auto;border-radius:1vw;background:white}.formula{text-align:center;font-size:2.4vw;color:#65d3c3}.road{display:flex;flex-wrap:wrap;gap:1vw}.road span{padding:1vw;border:1px solid #65d3c3;border-radius:999px;background:#184e77;font-size:1.25vw}footer{position:absolute;bottom:2.8vh;left:7vw;right:7vw;display:flex;justify-content:space-between;color:#9eb6c8}aside{display:none}body.notes aside{display:block;position:absolute;right:2vw;bottom:7vh;width:31vw;padding:1vw;background:#fff;color:#172033;border-radius:.7vw}@page{size:13.333in 7.5in;margin:0}@media print{html,body{width:13.333in;height:7.5in;overflow:visible}.slide{display:block!important;width:13.333in;height:7.5in;page-break-after:always}aside{display:none!important}}"""
    cards = []
    ...
    page = f'''<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Proyecto 1 · V5</title><style>{css}</style></head><body>{''.join(cards)}<script>const s=[...document.querySelectorAll('.slide')];let i=0;function show(n){{i=Math.max(0,Math.min(s.length-1,n));s.forEach((x,j)=>x.classList.toggle('active',j===i))}}document.onkeydown=e=>{{if(['ArrowRight',' ','PageDown'].includes(e.key))show(i+1);if(['ArrowLeft','PageUp'].includes(e.key))show(i-1);if(e.key.toLowerCase()==='n')document.body.classList.toggle('notes')}};show(0)</script></body></html>'''
    out_dir = ROOT / "entregables" / "presentacion"
    out_dir.mkdir(parents=True, exist_ok=True)
    out = out_dir / "presentacion.html"
    out.write_text(page, encoding="utf-8", newline="\n")
    candidates = [shutil.which("msedge"), Path(r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"), Path(r"C:\Program Files\Microsoft\Edge\Application\msedge.exe")]
    edge = next((str(x) for x in candidates if x and Path(x).exists()), None)
    if edge:
