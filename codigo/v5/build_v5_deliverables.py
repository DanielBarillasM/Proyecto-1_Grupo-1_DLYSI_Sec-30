...
def _build_report_legacy() -> Path:
    ...
    prevalence_tex = pct(R["datos"]["prevalencia"]).replace("%", r"\%")
...
\section*{{Resumen ejecutivo}} Se estudiaron {R['datos']['filas']:,} transacciones IEEE-CIS ({prevalence_tex} fraude). La investigación compara A, LightGBM sin lectura secuencial; B, GRU causal de 16 eventos; y C, fusión A+B. En evaluación interna, A obtiene AP {INTERNAL['A_V4']['auc_pr']:.4f}, B {INTERNAL['B_GRU']['auc_pr']:.4f} y C {INTERNAL['C_fusion']['auc_pr']:.4f}. A minimiza costo ({money(INTERNAL['A_V4']['cost_q'])}) y queda como candidato. Permutar B cambia AP de {FALS['original_internal']['auc_pr']:.4f} a {FALS['permutation_mean_auc_pr']:.4f}; no se atribuye valor al orden. El benchmark es histórico reutilizado, no test ciego.
