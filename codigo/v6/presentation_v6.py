"""Construye la presentación HTML ampliada de Proyecto 1 V6.
...
def build_presentation(
    ...
    """Escribe la presentación V6 y, si Edge está disponible, actualiza el PDF."""

    root = root.resolve()
    if results is None:
        results = json.loads(
            (root / "artefactos" / "v6" / "resultados_v6.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    replacements = {
        ...
        "__D_AP__": _f(d["auc_pr"]),
        "__D_RECALL__": _pct(d["recall"]),
        "__D_PRECISION__": _pct(d["precision"]),
        "__D_COST__": _money(d["cost_q"]),
        "__A_BENCH_AP__": _f(a_benchmark["auc_pr"]),
        "__A_BENCH_COST__": _money(a_benchmark["cost_q"]),
        "__A_THRESHOLD__": _f(results["umbrales"]["A"], 4),
        "__B_THRESHOLD__": _f(results["umbrales"]["B"], 4),
        "__C_THRESHOLD__": _f(results["umbrales"]["C"], 4),
...
<head>
  ...
  <title>Proyecto 1 · Monitoreo transaccional · V6</title>
  <style>
...
<body>
  <section class="slide" data-title="Decisión ejecutiva">
    <div class="topline"><div class="eyebrow">01 · Decisión ejecutiva</div><div class="chips"><span class="chip hot">V6 OFICIAL</span><span class="chip">IEEE-CIS</span><span class="chip">A/B/C/D</span></div></div>
    ...
    <aside class="notes"><h3>Mensaje principal</h3><p>Abra con la decisión, no con la arquitectura. Aclare que A es el modelo tabular V4 recalibrado dentro del protocolo V6. La conclusión no es que las GRU sean inútiles, sino que esta representación de identidad y este conjunto de variables no demostraron valor temporal adicional.</p><p><b>Transición:</b> para justificar la decisión, primero hay que explicar cómo se evitó entrenar con el futuro.</p></aside>
  </section>
  ...
  <section class="slide" data-title="Datos y protocolo">
    ...
    <div class="content">
      <div class="timeline">
        <div class="stage"><span class="label">Fuente</span><b>IEEE-CIS</b><p>Vesta Corporation · <a href="https://www.kaggle.com/competitions/ieee-fraud-detection/overview" style="color:#8ecae6">competencia oficial de Kaggle</a>; unión por <code>TransactionID</code>.</p></div><div class="flow-arrow">→</div>
        ...
        <div class="stage"><span class="label">Historia</span><b>≤ 32 eventos</b><p>Clave proxy: tarjeta + dirección. Solo antecedentes previos al evento objetivo.</p></div><div class="flow-arrow">→</div>
  ...
  <section class="slide" data-title="Diseño A/B/C">
    ...
    <div class="content">
      <div class="models">
        ...
        <div class="model b"><span class="label">Lee antecedentes</span><div class="model-tag">B</div><h2>GRU vs TCN causal</h2><div class="mini-flow"><span>variables ampliadas</span><span>categorías</span><span>hasta 32 eventos</span></div><p class="small">TCN gana en model select. BCE ponderada, AdamW, clipping y early stopping.</p></div>
        <div class="flow-arrow">→</div>
        <div class="model c"><span class="label">Apuesta de complementariedad</span><div class="model-tag">C</div><h2>Fusión A + B + D</h2><div class="mini-flow"><span>score A</span><span>score B</span><span>anomalía D</span><span>calidad</span></div><p class="small">Fusión condicionada entrenada en un bloque independiente.</p></div>
      </div>
      <div class="common-core"><b>Control D:</b> encoder–decoder PyTorch entrenado solo con legítimas. C debía ganar <b>≥ 0.01 AP</b>, reducir costo <b>≥ 5%</b> y mantener recall ≥ 0.75.</div>
    </div>
  ...
  <section class="slide" data-title="Resultados comparables">
    ...
    <div class="content chart-panel">
      <div class="figure"><img src="../../../evidencia/figuras/v6/01_comparacion_abc_validacion.png" alt="Comparación AUC-PR interna de A, B y C"></div>
      <div>
        <table class="metric-table"><thead><tr><th>Modelo</th><th>AP</th><th>Recall</th><th>F1</th><th>Costo</th></tr></thead><tbody>
          ...
          <tr><td>B · TCN causal</td><td>__B_AP__</td><td>__B_RECALL__</td><td>__B_F1__</td><td>__B_COST__</td></tr>
          <tr><td>C · Fusión</td><td>__C_AP__</td><td>__C_RECALL__</td><td>__C_F1__</td><td>__C_COST__</td></tr>
          <tr><td>D · Autoencoder</td><td>__D_AP__</td><td>__D_RECALL__</td><td>—</td><td>__D_COST__</td></tr>
        </tbody></table>
        <div class="grid2">
    ...
    <aside class="notes"><h3>Lectura correcta de la tabla</h3><p>A conserva el ranking defendible. C reduce levemente costo, pero pierde AP y no alcanza su regla. D logra recall generando demasiadas anomalías legítimas: precisión __D_PRECISION__. AP evalúa el ranking completo; precisión, recall, F1 y costo dependen del umbral.</p><p>El benchmark se presenta solo como referencia histórica; la decisión V6 se fijó en la evaluación interna.</p></aside>
  </section>
  ...
  <section class="slide" data-title="Falsificación del orden">
    ...
    <div class="content test-grid">
      <div class="figure"><img src="../../../evidencia/figuras/v6/03_falsificaciones_orden_v6.png" alt="Resultados de permutación y recorte de historia"></div>
      <div>
    ...
    <aside class="notes"><h3>La diapositiva clave</h3><p>La permutación conserva los eventos y deja la transacción objetivo al final; solo destruye el orden de antecedentes. Si la TCN usara una dependencia temporal útil, AP debería caer. En lugar de caer, sube.</p><p>No diga que toda secuencia es inútil. Diga que, con esta clave proxy, variables y arquitecturas probadas, no se demostró valor adicional del orden.</p></aside>
  </section>
  ...
  <section class="slide" data-title="Economía y umbral">
    ...
    <div class="content chart-panel">
      <div class="figure"><img src="../../../evidencia/figuras/v6/04_costos_abc_v6.png" alt="Costo económico de A, B y C"></div>
      <div>
  ...
  <section class="slide" data-title="Errores y controles">
    ...
    <div class="content operations">
      <div>
        ...
        <div class="figure" style="min-height:27vh;margin-top:.7vw"><img src="../../../evidencia/figuras/v6/05_calibracion_v6.png" alt="Curvas de calibración de A, B y C"></div>
      </div>
  ...
  <section class="slide" data-title="Recomendación y ruta">
    ...
    <div class="content roadmap">
      <div>
        ...
        <div class="final-box" style="margin-top:1vw"><div><span class="label">Candidato al Proyecto Final</span><h2 style="font-size:2vw;margin:.25vw 0;color:var(--mint)">A · LightGBM V4 + calibración V6</h2><p class="small">Fuente: IEEE-CIS Fraud Detection, Kaggle. Entrada contractual → score → umbral → revisión. Una cohorte nueva puede reabrir la decisión.</p></div><img class="qr" src="../../../evidencia/recursos/v6/qr_repositorio_v6.png" alt="Código QR del repositorio"></div>
      </div>
      <div class="card teal" style="display:flex;flex-direction:column;justify-content:space-between"><div><span class="label">Conclusión defendible</span><div class="verdict"><span class="dot"></span>CONSERVAR A</div><h2 style="font-size:2vw;margin:1vw 0">No migrar a B, C o D con la evidencia actual</h2><p class="small">La mejora plausible es mejor identidad, cohortes nuevas, categorías completas, costos reales y validación walk-forward; no solo una red mayor.</p></div><div class="callout"><span class="dot"></span><div><b>Evidencia:</b> A gana internamente; B no usa favorablemente el orden; C incumple el gate; D confunde anomalía con fraude.</div></div></div>
    </div>
  ...
  <script>
    ...
    output_dir = root / "entregables" / "presentacion" / "v6"
    ...
    if export_pdf:
        ...
        if edge:
            with tempfile.TemporaryDirectory(prefix="proyecto1_v6_edge_") as temp:
                temp_dir = Path(temp)
                temp_pdf = temp_dir / "presentacion_v6.pdf"
                subprocess.run(
                    [
                        edge,
                        "--headless",
