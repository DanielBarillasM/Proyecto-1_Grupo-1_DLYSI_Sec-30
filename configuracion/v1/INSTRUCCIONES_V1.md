...
```powershell
jupyter nbconvert --to notebook --execute --inplace --ExecutePreprocessor.timeout=600 entregables/cuaderno/v1/proyecto1_calderon_barillas.ipynb
```
...
La salida se guarda en `entregables/ficha/v1/Ficha_Repositorio_Proyecto1.docx`. El documento incluye un QR que apunta al repositorio.
...
```powershell
Start-Process entregables/presentacion/v1/presentacion.html
```
...
Ejecute `python codigo/compartido/download_data.py` y compruebe que ambos archivos estén en `datos/raw/`.
